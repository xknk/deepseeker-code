 ...
 *   - 卡片内不用 spinner 动画：逐帧 setTick 放在卡片里时，Ink 擦除失准会把上一帧叠在下面
 *     （表现为同一工具出现两行不同的 ⠋/⠸）。执行期的唯一逐帧动画收敛到底部「生成中」单行
 *     （App.tsx SPINNER_FRAMES，固定高度、不参与 Static 迁移），卡片区域保持零重绘无叠帧。
 *     卡内进度由「⎿ 运行中…」行（或实时 progress 尾行）静态表达。
