...
- **代码变更 diff 视图**：文件修改工具卡内联左右对比（红删绿增），点击放大全屏阅读（Esc / ✕ / 点空白关闭），亦可「⎇ 打开左右对比」唤起原生 `vscode.diff` 编辑器长期对照；不依赖 git。
- **两阶段计划模式**：先只读调研出方案 →「准备开始编码？」审批（是并自动接受编辑 / 是并手动审批编辑 / 编辑方案 / 否——留在计划模式继续迭代修订）→ 再落地实现；计划 / 自动模式下输入框带彩色边框与模式徽标。
