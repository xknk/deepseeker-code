...
新会话开新页签（多会话 Tab 架构）+ 修复消息流横向滚动空白。

### 新会话 = 新页签（对齐 Claude Code 多会话形态）

- 「新会话」（工具栏按钮 / `/new` / 命令面板）在**已有对话**的页签里触发时改为打开**新编辑器页签**，旧对话原样保留、可切回继续聊；空白页签原地复用，不堆积空页。
- 底层从单例 panel + 单 host 重构为多 Tab 架构：一个页签 = 一个 WebviewPanel + 一个独立 ChatHost（独立会话/计划/自动模式/模型），事件按页签定向投递互不串扰。
- 会话 id 持久化按活动页签收口：重载窗口后恢复最近活动的那个会话，其余经 /sessions 续接。关闭页签即中止该页签生成（防后台静默烧 token），transcript 已落盘不受影响。

### 修复：消息流横向滚动条滑过去是空白

- `#messages` 只设了 `overflow-y:auto`（overflow-x 被计算成 auto），助手行 flex item 无 `min-width:0`，长代码行的 `<pre>` 把整行撑到内容宽度，产生一条「滑过去是空白」的横向滚动条。现消息流显式禁止横向平移：长内容由内部 pre / 表格自滚动，info/system 行长 token 折行兜底，user/assistant 正文行可收缩换行。
