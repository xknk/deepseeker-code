...
export const handleUnifiedChat = async (
    ...
    //   必须在 buildContextMessages / appendMessage 之前拼入 inbound.content——这样 wire/archive 双视图
    //   都含注入文本（对齐上方斜杠展开的既有原则）；一个接缝同时覆盖 CLI（经 handleUnifiedChat）与 HTTP 宿主。
    ...
    try {
        ...
        const fullMessages: Msg[] = await buildContextMessages(
            ...
            sysPrompt,
            opts?.model, // ★ vision 自动判定与实际请求模型（opts.model ?? MODEL_NAME）同源
        );
        await emitTrace({
            sessionId,
        ...
        await appendMessage({ sessionId, role: 'user', content: archiveContent })
