...
export const askTools: CustomTool[] = [
    {
        ...
        function: {
            name: "ask_question",
            description: [
                "向用户提出一个多选问题并等待作答。用于需要用户拍板的决策（如技术方案二选一、实现路径取舍、范围确认）或澄清歧义——切勿用于能从代码/上下文自行确定的问题。",
                "提供 2-4 个选项，每个含简短 label 与可选 description。multiSelect=true 允许多选（默认单选）。",
                "用户的选择会作为结果返回；若用户取消，返回空选择（此时应改用默认或重新提问）。",
                "注意：仅在交互式环境可用；若结果提示「不支持」，改用纯文本向用户提问。",
            ].join("\n"),
            parameters: {
                type: "object",
                properties: {
