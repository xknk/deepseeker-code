...
export const recallTools: CustomTool[] = [
    {
        ...
        function: {
            name: "recall",
            description: "检索【本会话】全量历史转录（transcript），包括已被上下文压缩归档的原始消息与工具结果。使用时机：滚动摘要（⟦DSC:ARCHIVE-INDEX⟧/⟦DSC:ARCHIVE-NOTES⟧）不足以回答细节时——如归档前的文件内容、报错原文、用户早前的要求——先用本工具检索，不要凭摘要臆测，也不要重跑工具。★ 首查策略：query 必须用具体实体词——报错关键词（如 TypeError）、文件路径、函数/符号名（摘要实体索引里出现过什么就用什么）；不要用工具名（read_file 等）或泛词（代码/文件/问题）作 query，会命中大量无关工具行。一次没中就换更具体的词或用 is_regex 以 | 合并多个候选词，通常 1-2 次内命中。返回按时间旧→新（优先归档区）；文件类工具结果附实时校验：⚠️ 已修改/已不存在 的必须重新 read_file 验证后才能据以修改，✓ 未变 方可直接引用。结果曾被截断的会标注 with_full 可取存档全文。摘要已够用时勿调用（省 token）。",
            parameters: {
                type: "object",
                properties: {
